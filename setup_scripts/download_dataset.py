...
def download_file(url, path):
    ...
    # Deleting the partially downloaded file.
    if os.path.isfile(path):
        print(f'Deleted existing partial file {path}')
        os.remove(path)
...
def clear_screen():
    """Clears the console screen irrespective of the OS used."""
    import platform
    if platform.system() == 'Windows':
        os.system('cls')
    else:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=len(dataset_urls)) as executor:
        futures = []
        for url in dataset_urls:
            path = os.path.join(target_folder, f'{dataset_file_name}{dataset_part}.zip')
            print(f'\t\tDownloading: {path}')
